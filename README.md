...
| **[oj-statusline](./plugins/oj-statusline)** | 1.2.2 | Multi-line Claude Code statusline showing token breakdown (cached/new/total/ctx%), MCP and hook counts, and a soft-wrapped list of enabled skills with active-skill highlight. Token line always renders (even at session start), idempotent silent SessionStart auto-wire, and bundled `/oj-statusline:setup` slash command with cross-platform installer discovery. |
